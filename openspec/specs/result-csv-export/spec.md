...
The system SHALL export a header row and exactly these columns in order for every successful result: `source_file`, `datum`, `einsatzstichwort`, `ort`, `priority`, `alarm_text`, `einheit`, and `verfasser`.
...
- **WHEN** `ort`, `einheit`, or `verfasser` is absent or empty in a successful result
